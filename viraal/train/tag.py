...
class TrainJoint(TrainText):
    ...
    def instantiate_metrics(self):
        m = {
            'labeled_train': [metrics.F1Conlleval('tag'), metrics.Average('ce_loss'), metrics.Average('vat_loss')],
            'unlabeled_train': [metrics.F1Conlleval('tag'), metrics.Average('vat_loss')],
            'train': [metrics.F1Conlleval('tag'), metrics.Average('vat_loss')],
            'val' : [metrics.F1Conlleval('tag')],
            'test' : [metrics.F1Conlleval('tag')]
        }
    ...
    def train_epoch(self):
        ...
        for batch in self.get_iterator():
            ...
            tag_logits = self.model(embeddings=embeddings, mask=mask)
            tags = batch["tags"]
            labeled = np.array(batch["labeled"], dtype=bool)

            if "ce" in self.losses and any(labeled):
                ce_loss = self.losses["ce"](tag_logits[labeled][mask[labeled]], tags[labeled][mask[labeled]]).mean()
            ...
            if "vat" in self.losses:
                ...
                vat_tag_loss = self.losses["vat"](tag_logits, model_forward, embeddings, mask)
                vat_tag_loss = vat_tag_loss.mean()
                vat_tag_loss.backward()
                self.metrics.update("train", vat_loss=vat_tag_loss)
            
            tensors = from_locals(["int_logits", "tag_logits", "tags", "mask"], loc=locals())
...
@hydra.main(config_path="../config/train_tag.yaml", strict=False)
def train_text(cfg):
